...
class GameReader:
    ...
    @property
    def players_globals_address(self):
        return self.mr.read_u32(0x2FAD20)

    @property
    def teams_address(self):
        return self.mr.read_u32(0x2FAD24)

    @property
    def game_globals_address(self):
        return self.mr.read_u32(0x27629C)

    @property
    def global_game_globals_address(self):
        return self.mr.read_u32(0x39BE4C)

    @property
    def game_server_address(self):
        return self.mr.read_u32(0x2E3628)

    @property
    def game_client_address(self):
        return self.mr.read_u32(0x2E362C)

    @property
    def game_connection_address(self):
        return (
            0x2E3684  # ? This wasn't read, assuming it's a constant address - Stewball
        )

    @property
    def is_team_game_address(self):
        return self.mr.read_u8(0x2F90C4)

    @property
    def game_time_globals_address(self):
        return self.mr.read_u32(0x2F8CA0)

    @property
    def global_tag_instances_address(self):
        return self.mr.read_u32(0x39CE24)

    @property
    def hud_messages_pointer(self):
        return self.mr.read_u32(0x276B40)

    @property
    def something_saying_main_menu(self):
        return self.mr.read_u32(0x2E4000 + 4)

    @property
    def player_datum_array(self):
        return int(self.mr.read_u32(0x2FAD28))

    @property
    def player_datum_array_max_count(self):
        return self.mr.read_u16(self.player_datum_array + 0x20)

    @property
    def player_datum_array_element_size(self):
        return self.mr.read_u16(self.player_datum_array + 0x22)

    @property
    def player_datum_array_first_element_address(self):
        return self.mr.read_u32(self.player_datum_array + 0x34)

    @property
    def game_time_address(self):
        return self.game_time_globals_address + 12

    @property
    def global_scenario_address(self):
        return int(self.mr.read_u32(0x39BE5C))

    @property
    def spawn_count(self):
        return self.mr.read_u32(self.global_scenario_address + 852)

    @property
    def first_spawn_address(self):
        return self.mr.read_u32(self.global_scenario_address + 856)

    spawns_cache = []

    @property
    def spawns(self):
        if self.spawns_cache:
            return self.spawns_cache

        # FIXME removed because the iterator will have the same result
        # May re-add if we don't want to cachce on []
        # if self.spawn_count <= 0:
        #     return []

        self.spawns_cache = [
            {
                "address": f"{hex(spawn_address)} -> {hex(self.mr.get_host_address(spawn_address))}",
                "spawn_id": spawn_index,
                "x": self.mr.read_float(spawn_address),
                "y": self.mr.read_float(spawn_address + 4),
                "z": self.mr.read_float(spawn_address + 8),
                "facing": self.mr.read_float(spawn_address + 12),
                "team_index": self.mr.read_u8(spawn_address + 16),
                "bsp_index": self.mr.read_u8(spawn_address + 17),
                "unk0": hex(self.mr.read_u16(spawn_address + 18)),
                "gametypes": [
                    self.mr.read_u8(spawn_address + 20),
                    self.mr.read_u8(spawn_address + 21),
                    self.mr.read_u8(spawn_address + 22),
                    self.mr.read_u8(spawn_address + 23),
                ],
            }
            for spawn_index in range(self.spawn_count)
            if (
                spawn_address := self.first_spawn_address + 52 * spawn_index
    ...
    def get_spawns(self, cache_results=True):
        ...
        global_scenario_address = self.mr.read_u32(0x39BE5C)
        spawn_count = self.mr.read_u32(global_scenario_address + 852)
        first_spawn_address = self.mr.read_u32(global_scenario_address + 856)
    ...
    def get_items(self, cache_results=True):
        ...
        global_scenario_address = self.mr.read_u32(0x39BE5C)
        item_count = self.mr.read_s32(global_scenario_address + 900)
        first_item_address = self.mr.read_u32(global_scenario_address + 904)
        ...
        for item_index in range(item_count):
            ...
            unknown_item_attribute = self.mr.read_s16(item_address + 0xE)

            # Filter items based on unknown_item_attribute if needed
            if unknown_item_attribute is None:
                continue

            tag_index = self.mr.read_s32(item_address + 0x5C)
            if tag_index == -1:
                continue

            tag_index_short = tag_index & 0xFFFF
            tag_name_address = self.global_tag_instances_address + 32 * tag_index_short
            tag_name = self.mr.read_string(self.mr.read_s32(tag_name_address + 0x10))
            item_spawn_interval = self.mr.read_s16(
                self.mr.read_s32(tag_name_address + 0x14) + 0xC
            )

            # Create item dictionary and append to the list
            item = {
                "address": f"{hex(item_address)} -> {hex(self.mr.get_host_address(item_address))}",
                "tag_id": tag_index_short,
                "tag_name": tag_name,
                "item_spawn_interval": item_spawn_interval,
                "item_game_type": self.mr.read_u8(item_address + 0x4),
                "item_x": self.mr.read_float(item_address + 0x40),
                "item_y": self.mr.read_float(item_address + 0x44),
                "item_z": self.mr.read_float(item_address + 0x48),
            }
            items.append(item)
    ...
    def get_game_time_info(self):
        ...
        game_time_info = dict(
            ...
            game_time_initialized=self.mr.read_u8(gtga),
            game_time_active=self.mr.read_u8(gtga + 1),
            game_time_paused=self.mr.read_u8(gtga + 2),
            game_time_monitor_state=self.mr.read_s16(gtga + 4),
            game_time_monitor_counter=self.mr.read_s16(gtga + 6),
            game_time_monitor_latency=self.mr.read_s16(gtga + 8),
            game_time=self.mr.read_u32(gtga + 12)
            - 1,  # gets incremented after game engine is done, so we really want game_time-1
            game_time_elapsed=self.mr.read_u32(
                gtga + 16
            ),  # looks like elapsed time in last drawn frame (dropped frame count)
            game_time_speed=self.mr.read_float(gtga + 24),  # 1.0 is normal speed
            game_time_leftover_dt=self.mr.read_float(gtga + 28),
            update_client_maximum_actions=self.mr.read_u32(0x2E87E8)
            - self.mr.read_u32(0x2E87E4)
            + 1,  # typically gets set to 1 then decremented back to 0
            game_time_globals_address_hex=f'{gtga:#x} -> {self.mr.known_addresses[gtga]["host_address"]:#x}',
            real_time_elapsed=str(
                datetime.timedelta(seconds=self.mr.read_u32(gtga + 12) / 30)
            ).split(".")[
                0
    ...
    def get_key_data(self):
        ...
        return dict(
            # kernel_header=self.mr.get_formatted_bytes(0x80010000, 200),
    ...
    def object_string_from_type(self, object_type):
        ...
        type_def_addr = self.mr.read_u32(
            object_type_definitions_array + 4 * object_type
        )
        type_string = self.mr.read_string(self.mr.read_u32(type_def_addr))
        return type_string
    ...
    def get_objects(self):
        ...
        objects = []
        object_header_datum_array = self.mr.read_u32(0x2FC6AC)
        object_header_datum_array_total_count = self.mr.read_u16(
            object_header_datum_array + 0x2E
        )
        object_header_datum_array_first_element_address = self.mr.read_u32(
        ...
        object_datum_size = self.mr.read_u16(0x1FC0E0)
        unit_datum_size = self.mr.read_u16(0x1FC188)
        item_datum_size = self.mr.read_u16(0x1FC380)

        for i in range(object_header_datum_array_total_count):
            base_address = object_header_datum_array_first_element_address + 12 * i
            object_address = self.mr.read_u32(base_address + 8)
            if object_address == 0x0:
                continue

            # Gather basic object information
            tag_index = self.mr.read_s16(object_address)
            tag_name = self.mr.read_string(
                self.mr.read_u32(
                    32 * tag_index + self.global_tag_instances_address + 0x10
                )
            )
            object_type = self.mr.read_u8(object_address + 0x64)
            object_type_string = self.object_string_from_type(object_type)

            # Object details
            obj_details = {
                "object_id": i,
                "address": f'{hex(object_address)} -> {hex(self.mr.known_addresses[object_address]["host_address"])}',
                # "header_data": self.mr.get_formatted_bytes(base_address, 12),
                "flags": hex(self.mr.read_u32(object_address + 0x4)),
                "x": self.mr.read_float(object_address + 0xC),
                "y": self.mr.read_float(object_address + 0x10),
                "z": self.mr.read_float(object_address + 0x14),
                "vel_x": self.mr.read_float(object_address + 0x18),
                "vel_y": self.mr.read_float(object_address + 0x1C),
                "vel_z": self.mr.read_float(object_address + 0x20),
                "ang_vel_x": self.mr.read_float(object_address + 0x3C),
                "ang_vel_y": self.mr.read_float(object_address + 0x40),
                "ang_vel_z": self.mr.read_float(object_address + 0x44),
                "time_existing": self.mr.read_s16(object_address + 0x6C),
                "unk_damage_1": self.mr.read_s16(object_address + 0x68),
                "owner_unit_ref": hex(self.mr.read_u32(object_address + 0x70)),
                "owner_object_ref": hex(self.mr.read_u32(object_address + 0x74)),
                "parent_ref": hex(self.mr.read_u32(object_address + 0xCC)),
                "ultimate_parent": hex(self.mr.read_u32(object_address + 0x1E4)),
                "state_flags": self.mr.read_u8(object_address + 0x1A4),
                "drop_time": self.mr.read_u32(object_address + 0x1B4),
            ...
            if object_type_string == "projectile":
                ...
                obj_details["type_specific_data"] = {
                    "flags": self.mr.read_u32(projectile_address),
                    "address": f'{hex(projectile_address)} -> {hex(self.mr.known_addresses[projectile_address]["host_address"])}',
                    "action": self.mr.read_s16(projectile_address + 0x4),
                    "hit_material_type": self.mr.read_s16(projectile_address + 0x6),
                    "ignore_object_index": self.mr.read_s32(projectile_address + 0x8),
                    "target_object_index": self.mr.read_s32(projectile_address + 0x1C),
                    "detonation_timer": self.mr.read_float(projectile_address + 0x14),
                    "detonation_timer_delta": self.mr.read_float(
                        projectile_address + 0x18
                    ),
                    "arming_time": self.mr.read_float(projectile_address + 0x1C),
                    "arming_time_delta": self.mr.read_float(projectile_address + 0x20),
                    "distance_traveled": self.mr.read_float(projectile_address + 0x24),
                    "deceleration_timer": self.mr.read_float(projectile_address + 0x28),
                    "deceleration_timer_delta": self.mr.read_float(
                        projectile_address + 0x2C
                    ),
                    "deceleration": self.mr.read_float(projectile_address + 0x30),
                    "maximum_damage_distance": self.mr.read_float(
                        projectile_address + 0x34
                    ),
                    "rotation_axis_x": self.mr.read_float(projectile_address + 0x3C),
                    "rotation_axis_y": self.mr.read_float(projectile_address + 0x40),
                    "rotation_axis_z": self.mr.read_float(projectile_address + 0x44),
                    "rotation_sine": self.mr.read_float(projectile_address + 0x48),
                    "rotation_cosine": self.mr.read_float(projectile_address + 0x4C),
                }
    ...
    def get_flag_data(self):
        game_engine_globals_address = self.mr.read_u32(0x2F9110)
        if (
            game_engine_globals_address
            and self.mr.read_u32(game_engine_globals_address + 0x4) == 1
        ):
            flag_0 = self.mr.read_u32(0x2762A4)
            flag_1 = self.mr.read_u32(0x2762A4 + 4)
            return dict(
                flag_base_0=dict(
                    x=self.mr.read_float(flag_0),
                    y=self.mr.read_float(flag_0 + 4),
                    z=self.mr.read_float(flag_0 + 8),
                ),
                flag_base_1=dict(
                    x=self.mr.read_float(flag_1),
                    y=self.mr.read_float(flag_1 + 4),
                    z=self.mr.read_float(flag_1 + 8),
                ),
            )
        return {}
    ...
    def get_fog(self):
        ...
        fog_params = dict(
            fog_params_address=f"{hex(0x2FC8A8)} -> {hex(self.mr.get_host_address(0x2FC8A8))}",
            fog_color_r=self.mr.read_float(fog_params_address + 0x4),
            fog_color_g=self.mr.read_float(fog_params_address + 0x8),
            fog_color_b=self.mr.read_float(fog_params_address + 0xC),
            fog_max_density=self.mr.read_float(fog_params_address + 0x10),
            fog_atmo_min_dist=self.mr.read_float(
                fog_params_address + 0x14
            ),  # defaults to 1024?
            fog_atmo_max_dist=self.mr.read_float(
    ...
    def get_memory_info(self):
        ...
        memory_info = dict(
            game_state_base_address=f"{hex(self.mr.read_u32(0x2E2D14))} -> {hex(self.mr.get_host_address(self.mr.read_u32(0x2E2D14)))}",
            tag_cache_base_address=f"{hex(self.mr.read_u32(0x2E2D18))} -> {hex(self.mr.get_host_address(self.mr.read_u32(0x2E2D18)))}",
            texture_cache_base_address=f"{hex(self.mr.read_u32(0x2E2D1C))} -> {hex(self.mr.get_host_address(self.mr.read_u32(0x2E2D1C)))}",
            sound_cache_base_address=f"{hex(self.mr.read_u32(0x2E2D20))} -> {hex(self.mr.get_host_address(self.mr.read_u32(0x2E2D20)))}",
            game_state_size=f"{hex(self.mr.read_u32(0x32E4A))}",
            tag_cache_size=f"{hex(self.mr.read_u32(0x32E5D))}",
            texture_cache_size=f"{hex(self.mr.read_u32(0x32E75))}",
            sound_cache_size=f"{hex(self.mr.read_u32(0x32E8A))}",
        )
    ...
    def get_player_ui_globals(self, local_player):
        ...
        return dict(
            address=hex(
                self.mr.get_host_address(player_ui_globals_address + local_player * 56)
            ),
            # TODO: profile name is at +0 widechar
            color=self.mr.read_u8(player_ui_globals_address + local_player * 56 + 24),
            button_config=self.mr.read_u8(
                player_ui_globals_address + local_player * 56 + 40
            ),
            joystick_config=self.mr.read_u8(
                player_ui_globals_address + local_player * 56 + 41
            ),
            sensitivity=self.mr.read_u8(
                player_ui_globals_address + local_player * 56 + 42
            ),
            joystick_inverted=self.mr.read_u8(
                player_ui_globals_address + local_player * 56 + 43
            ),
            rumble_enabled=self.mr.read_u8(
                player_ui_globals_address + local_player * 56 + 44
            ),
            flight_inverted=self.mr.read_u8(
                player_ui_globals_address + local_player * 56 + 45
            ),
            autocenter_enabled=self.mr.read_u8(
                player_ui_globals_address + local_player * 56 + 46
            ),
            active_player_profile_index=f"{hex(self.mr.read_u32(player_ui_globals_address + local_player * 56 + 48))}",  # used for saving profile data
            joined_multiplayer_game=self.mr.read_u8(
    ...
    def get_input_data(self, local_player_index, player_id):
        ...
        player_control_address = self.mr.read_u32(0x276794)
        update_client_player_base = self.mr.read_u32(0x2E8870)
        update_client_player_address = self.mr.read_u32(
            update_client_player_base + 0x34
        )
        player_offset = 0x28 * player_id
        local_player_offset = 0x1C * local_player_index
        button_field = self.mr.read_u8(
            update_client_player_address + player_offset + 0x4
        )
        action_field = self.mr.read_u8(
        ...
        player_control_state = (
            {
                "player_desired_yaw": self.mr.read_float(
                    (local_player_index << 6) + player_control_address + 0x1C
                ),
                "player_desired_pitch": self.mr.read_float(
                    (local_player_index << 6) + player_control_address + 0x20
                ),
                "player_zoom_level": self.mr.read_s16(
                ...
                "player_aim_assist_near": self.mr.read_float(
                    (local_player_index << 6) + player_control_address + 16 + 0x2C
                ),
                "player_aim_assist_far": self.mr.read_float(
        ...
        input_abstraction_input_state = (
            {
                "address": f"{hex(self.mr.get_host_address(0x2E4600))}",
                "a": self.mr.read_u8(0x2E4600 + local_player_offset + 0x0),
                "black": self.mr.read_u8(0x2E4600 + local_player_offset + 0x1),
                "x": self.mr.read_u8(0x2E4600 + local_player_offset + 0x2),
                "y": self.mr.read_u8(0x2E4600 + local_player_offset + 0x3),
                "b": self.mr.read_u8(0x2E4600 + local_player_offset + 0x4),
                "white": self.mr.read_u8(0x2E4600 + local_player_offset + 0x5),
                "left_trigger": self.mr.read_u8(0x2E4600 + local_player_offset + 0x6),
                "right_trigger": self.mr.read_u8(0x2E4600 + local_player_offset + 0x7),
                "start": self.mr.read_u8(0x2E4600 + local_player_offset + 0x8),
                "back": self.mr.read_u8(0x2E4600 + local_player_offset + 0x9),
                "left_stick_button": self.mr.read_u8(
                    0x2E4600 + local_player_offset + 0xA
                ),
                "right_stick_button": self.mr.read_u8(
                    0x2E4600 + local_player_offset + 0xB
                ),
                "left_stick_vertical": self.mr.read_float(
                    0x2E4600 + local_player_offset + 0xC
                ),
                "left_stick_horizontal": self.mr.read_float(
                    0x2E4600 + local_player_offset + 0x10
                ),
                "right_stick_horizontal": self.mr.read_float(
                    0x2E4600 + local_player_offset + 0x14
                ),
                "right_stick_vertical": self.mr.read_float(
        ...
        input_gamepad_state = (
            {
                "address": f"{hex(self.mr.get_host_address(0x276AFC + player_offset))}",
                "address2": f"{hex(self.mr.get_host_address(0x276A5C + player_offset))}",
                "a": self.mr.read_u8(0x276A5C + player_offset + 0x0),
                "b": self.mr.read_u8(0x276A5C + player_offset + 0x1),
                "x": self.mr.read_u8(0x276A5C + player_offset + 0x2),
                "y": self.mr.read_u8(0x276A5C + player_offset + 0x3),
                "black": self.mr.read_u8(0x276A5C + player_offset + 0x4),
                "white": self.mr.read_u8(0x276A5C + player_offset + 0x5),
                "left_trigger": self.mr.read_u8(0x276A5C + player_offset + 0x6),
                "right_trigger": self.mr.read_u8(0x276A5C + player_offset + 0x7),
                "a_duration": self.mr.read_u8(0x276A5C + player_offset + 0x10),
                "b_duration": self.mr.read_u8(0x276A5C + player_offset + 0x11),
                "x_duration": self.mr.read_u8(0x276A5C + player_offset + 0x12),
                "y_duration": self.mr.read_u8(0x276A5C + player_offset + 0x13),
                "black_duration": self.mr.read_u8(0x276A5C + player_offset + 0x14),
                "white_duration": self.mr.read_u8(0x276A5C + player_offset + 0x15),
                "left_trigger_duration": self.mr.read_u8(
                    0x276A5C + player_offset + 0x16
                ),
                "right_trigger_duration": self.mr.read_u8(
                    0x276A5C + player_offset + 0x17
                ),
                "dpad_up_duration": self.mr.read_u8(0x276A5C + player_offset + 0x18),
                "dpad_down_duration": self.mr.read_u8(0x276A5C + player_offset + 0x19),
                "dpad_left_duration": self.mr.read_u8(0x276A5C + player_offset + 0x1A),
                "dpad_right_duration": self.mr.read_u8(0x276A5C + player_offset + 0x1B),
                "left_stick_duration": self.mr.read_u8(0x276A5C + player_offset + 0x1E),
                "right_stick_duration": self.mr.read_u8(
                    0x276A5C + player_offset + 0x1F
                ),
                "left_stick_horizontal": self.mr.read_s16(
                    0x276A5C + player_offset + 0x20
                ),
                "left_stick_vertical": self.mr.read_s16(
                    0x276A5C + player_offset + 0x22
                ),
                "right_stick_horizontal": self.mr.read_s16(
                    0x276A5C + player_offset + 0x24
                ),
                "right_stick_vertical": self.mr.read_s16(
        ...
        update_queue_values = {
            "address": f"{hex(self.mr.get_host_address(update_client_player_address + player_offset))}",
            "unit_ref": f"{hex(self.mr.read_u16(update_client_player_address + player_offset))}",
        ...
        return {
            ...
            "look_yaw_rate": self.mr.read_float(0x2E4684 + 4 * local_player_index),
            "look_pitch_rate": self.mr.read_float(0x2E4694 + 4 * local_player_index),
            "input_abstraction_globals": f"{hex(self.mr.read_u32(0x2E45A0))} @ {hex(self.mr.get_host_address(0x2E45A0))}",
            "player_control_pointer": f"{hex(player_control_address)} @ {hex(self.mr.get_host_address(0x276794))}",
            "player_control": f"{hex(self.mr.read_u32(player_control_address))} @ {hex(self.mr.get_host_address(player_control_address))}",
    ...
    def get_first_person_weapon(self, local_player_index):
        ...
        weapon_address = self.mr.read_u32(0x276B48) + 7840 * local_player_index

        return dict(
            address=f"{weapon_address:#x} -> {self.mr.get_host_address(weapon_address):#x}",
            weapon_rendered=self.mr.read_u32(
                weapon_address
            ),  # TODO: confirm if this is actually weapon_rendered
            player_object=f"{self.mr.read_u32(weapon_address + 4):#x}",  # player object id?
            weapon_object=f"{self.mr.read_u32(weapon_address + 8):#x}",  # weapon object id?
            state=self.mr.read_s16(weapon_address + 12),
            idle_animation_threshold=self.mr.read_s16(weapon_address + 14),
            idle_animation_counter=self.mr.read_s16(weapon_address + 16),
            animation_id=self.mr.read_s16(
                weapon_address + 22
            ),  # TODO: not sure if this is animation id or something else
            animation_tick=self.mr.read_s16(weapon_address + 24),
        )
    ...
    def get_observer_camera_info(self, local_player_index):
        ...
        return dict(
            address=f"{observer_camera_address:#x} -> {self.mr.get_host_address(observer_camera_address):#x}",
            x=self.mr.read_float(observer_camera_address),
            y=self.mr.read_float(observer_camera_address + 4),
            z=self.mr.read_float(observer_camera_address + 8),
            x_vel=self.mr.read_float(
                observer_camera_address + 20
            ),  # NOTE: these are different than player velocities (roughly player_vel * pi?)
            y_vel=self.mr.read_float(observer_camera_address + 24),
            z_vel=self.mr.read_float(observer_camera_address + 28),
            x_aim=self.mr.read_float(observer_camera_address + 32),
            y_aim=self.mr.read_float(observer_camera_address + 36),
            z_aim=self.mr.read_float(observer_camera_address + 40),
            fov=self.mr.read_float(
    ...
    def get_model_nodes(self, base_address):
        ...
        for offset in model_node_offsets:
            model_nodes.append(
                (
                    self.mr.read_float(base_address + offset),
                    self.mr.read_float(base_address + offset + 4),
                    self.mr.read_float(base_address + offset + 8),
                )
            )
    ...
    def get_game_variant_global(self):
        ...
        return dict(
            address=hex(self.mr.get_host_address(game_variant_global_address)),
            # values=self.mr.get_formatted_bytes(game_variant_global_address, 0x68),
        )
    ...
    def get_network_game_data(self, network_game_data_address):
        ...
        return dict(
            ...
            maximum_player_count=self.mr.read_u8(network_game_data_address + 270),
            machine_count=machine_count,
            network_machines=[
                dict(
                    # name=self.mr.read_wchar(network_machines_address + 68 * i),
                    machine_index=self.mr.read_u8(
            ...
            network_players=[
                dict(
                    # name=self.mr.read_wchar(network_players_address + 32 * i, 24),
                    color=self.mr.read_s16(network_players_address + 32 * i + 24),
                    unused=self.mr.read_s16(network_players_address + 32 * i + 26),
                    machine_index=self.mr.read_u8(
                        network_players_address + 32 * i + 28
                    ),
                    controller_index=self.mr.read_u8(
                        network_players_address + 32 * i + 29
                    ),
                    team=self.mr.read_u8(network_players_address + 32 * i + 30),
                    player_list_index=self.mr.read_u8(
    ...
    def get_network_game_client(self):
        ...
        return dict(
            machine_index=self.mr.read_u16(network_game_client_address),
            advertised_games=dict(),  # 9 games
            ping_target_ip=hex(self.mr.read_s32(network_game_client_address + 2056)),
            packets_sent=self.mr.read_s16(network_game_client_address + 2084),
            packets_received=self.mr.read_s16(network_game_client_address + 2086),
            average_ping=self.mr.read_s16(network_game_client_address + 2088),
            ping_active=self.mr.read_u8(network_game_client_address + 2090),
            seconds_to_game_start=self.mr.read_s16(network_game_client_address + 3236),
    ...
    def get_network_game_server(self):
        ...
        return dict(
            address=hex(self.mr.get_host_address(network_game_server_address)),
            # values=self.mr.get_formatted_bytes(network_game_server_address, 1212),
            countdown_active=self.mr.read_u8(network_game_server_address + 1172),
            countdown_paused=self.mr.read_u8(network_game_server_address + 1173),
            countdown_adjusted_time=self.mr.read_u8(network_game_server_address + 1174),
        )
    ...
    def dump_game_update_contents(self):
        ...
        self.mr.get_host_address(packet_data_address)
        self.mr.add_to_cache(packet_data_address, 5000)
        self.mr.get_host_address(data_queue)
        self.mr.add_to_cache(data_queue, 520)

        # Read update queue address
        update_queue_address = self.mr.read_u32(0x2E8870)
        update_client_player_address = self.mr.read_u32(update_queue_address + 0x34)
        ...
        u32_2E8870 = self.mr.read_u32(0x2E8870)
        host_u32_2E8870 = self.mr.get_host_address(u32_2E8870)
        u32_2E87E4 = self.mr.read_u32(0x2E87E4)
        u32_2E87E8 = self.mr.read_u32(0x2E87E8)

        return {
            "packet_data_address": f"{packet_data_address:#x} -> {self.mr.get_host_address(packet_data_address):#x}",
            "network_game_client": f"{network_game_client:#x} -> {self.mr.get_host_address(network_game_client):#x}",
            "data_queue_address": f"{data_queue:#x} -> {self.mr.get_host_address(data_queue):#x}",
            ...
            "dword_2E8874": hex(self.mr.read_u32(0x2E8874)),
            "dword_2E8870_plus46": hex(self.mr.read_s16(u32_2E8870 + 46)),
            "dword_2E8870_plus52": f"{hex(self.mr.read_u32(u32_2E8870 + 52))} -> {hex(self.mr.get_host_address(self.mr.read_u32(u32_2E8870 + 52)))}",
            "header": {
                "tick": self.mr.read_s32(data_queue),
                "global_random": hex(self.mr.read_u32(data_queue + 4)),
                "tick_2": self.mr.read_s32(data_queue + 8),
                "unk_1": self.mr.read_u16(data_queue + 12),  # player index?
                "player_count": self.mr.read_s16(data_queue + 14),
            },
            "data": {
                # Placeholder for additional data elements (uncomment if needed)
            },
            # "update_queue_header": self.mr.get_formatted_bytes(u32_2E8870, 0x34),
            "update_queue_values": {
                "unk_1": self.mr.read_s16(
                    update_queue_address + 0x20
                ),  # max element count?
                "unk_2": self.mr.read_s16(
                    update_queue_address + 0x22
                ),  # element length?
                "unk_3": self.mr.read_s16(update_queue_address + 0x24),  # not sure
                "unk_4": self.mr.read_s16(
                    update_queue_address + 0x2E
                ),  # element count?
                "unk_5": self.mr.read_s16(
                    update_queue_address + 0x30
                ),  # also element count?
                "unk_6": f"{hex(self.mr.read_u16(update_queue_address + 0x32))} ({self.mr.read_u16(update_queue_address + 0x32)})",
            },
            "queue_ids": " ".join(
                hex(
                    self.mr.read_u16(
    ...
    def get_animation_debug_info(self, unk_handle, animation_id, animation_tick):
        ...
        tag_address = self.mr.read_u32(
            32 * (unk_handle & 0xFFFF) + self.global_tag_instances_address + 20
        )
        animation_address = self.mr.read_u32(tag_address + 120) + 180 * animation_id

        animation_length = self.mr.read_s16(animation_address + 34)
        unk_46 = self.mr.read_s16(animation_address + 46)
        unk_52 = self.mr.read_s16(animation_address + 52)
        unk_54 = self.mr.read_s16(animation_address + 54)
    ...
    def get_game_info(self) -> dict:
        ...
        player_count = self.mr.read_u16(self.player_datum_array + 0x2E)
        player_stat_array = []
        ...
        game_time = self.mr.read_u32(self.game_time_globals_address + 12)
        game_time_elapsed = self.mr.read_u32(self.game_time_globals_address + 16)
        # print(game_time, game_time_elapsed)

        game_time_initialized = self.mr.read_u8(self.game_time_globals_address)
        game_time_active = self.mr.read_u8(self.game_time_globals_address + 1)
        game_time_paused = self.mr.read_u8(self.game_time_globals_address + 2)
        game_time_speed = self.mr.read_float(
            self.game_time_globals_address + 24
        )  # 1.0 is normal speed
        game_time_leftover_dt = self.mr.read_float(self.game_time_globals_address + 28)

        game_globals_map_loaded = self.mr.read_u8(self.game_globals_address)
        game_globals_active = self.mr.read_u8(self.game_globals_address + 1)

        main_menu_is_active = self.mr.read_u8(0x2E4068)
        ...
        game_connection = self.mr.read_u16(self.game_connection_address)
        ...
        object_header_datum_array = self.mr.read_u32(0x2FC6AC)
        object_header_datum_array_max_elements = self.mr.read_u16(
            object_header_datum_array + 0x20
        )
        object_header_datum_array_element_size = self.mr.read_u16(
            object_header_datum_array + 0x22
        )
        object_header_datum_array_allocated_object_count = self.mr.read_u16(
            object_header_datum_array + 0x2E
        )
        object_header_datum_array_element_count = self.mr.read_u16(
        ...
        if game_time_initialized and game_time_active and not main_menu_is_active:
            ...
            for player_index in range(player_count):
                ...
                player_object_handle = self.mr.read_s32(static_player_address + 0x34)
                previous_player_object_handle = self.mr.read_s32(
                ...
                dynamic_player_address = self.mr.read_u32(
                ...
                previous_dynamic_player_address = self.mr.read_u32(
                    object_header_datum_array_first_element_address
                ...
                player_object_debug = dict(
                    ...
                    object_header_datum_array=f'{hex(self.mr.read_u32(object_header_datum_array))} @ {hex(object_header_datum_array)} -> {hex(self.mr.known_addresses[object_header_datum_array]["host_address"])}',
                    object_header_datum_array_first_element_address=hex(
                        object_header_datum_array_first_element_address
                    ),
                    dynamic_player_address=(
                        f"{hex(dynamic_player_address)} -> {hex(self.mr.get_host_address(dynamic_player_address))}"
                        if player_object_handle != -1
                        else ""
                    ),
                    player_object_id=player_object_id,
                    static_player_address=f"{hex(static_player_address)} -> {hex(self.mr.get_host_address(static_player_address))}",
                ...
                player_object_debug["damage_table_address"] = (
                    f"{hex(damage_table_address)} -> {hex(self.mr.get_host_address(damage_table_address))}"
                )
                damage_table = []
                for i in range(4):
                    damage_time = self.mr.read_u32(damage_table_address + 16 * i)
                    if damage_time != 0xFFFFFFFF:
                        damage_amount = self.mr.read_float(
                            damage_table_address + 16 * i + 4
                        )
                        static_player = self.mr.read_u32(
                        ...
                        damage_table.append(
                            dict(
                                ...
                                dynamic_player=self.mr.read_u32(
                        ...
                        last_death = self.mr.read_u32(static_player_address + 0x84)
                ...
                if player_object_handle != -1:
                    ...
                    def get_weapon(weapon_object_handle):
                        ...
                        weapon_object_address = self.mr.read_u32(
                            self.mr.read_u32(object_header_datum_array + 52)
                        ...
                        if weapon_object_address == 0x0:
                            return {}
                        tag_address = (
                            32 * self.mr.read_s16(weapon_object_address)
                            + self.global_tag_instances_address
                        )
                        weapon_type = self.mr.read_u8(
                            self.mr.read_u32(tag_address + 20) + 0x309
                        )
                        ...
                        return dict(
                            ...
                            heat_meter=self.mr.read_float(
                                weapon_object_address + 0xD4
                            ),  # FIXME: seems to also be used for human weapons, need to figure out what
                            used_energy=self.mr.read_float(
                                weapon_object_address + 0xE0
                            ),  # only if energy weapon
                            charge_amount=self.mr.read_float(
                                weapon_object_address + 0xF0
                            ),  # remaining energy for PR, current overcharge for PP
                            reloading=self.mr.read_u8(
                                weapon_object_address + 0x258
                            ),  # 1 while reloading until reload_time hits 2
                            can_fire=self.mr.read_u8(weapon_object_address + 0x259),
                            reload_time=self.mr.read_s16(weapon_object_address + 0x25A),
                            backpack_ammo_count=self.mr.read_s16(
                                weapon_object_address + 0x25E
                            ),
                            magazine_ammo_count=self.mr.read_s16(
                                weapon_object_address + 0x260
                            ),
                            weapon_tag_address=f'{self.mr.read_u32(tag_address)} @ {hex(tag_address)} -> {hex(self.mr.known_addresses[tag_address]["host_address"])}',
                            ...
                            energy_used=self.mr.read_float(
                            ...
                            zoom_levels=self.mr.read_s16(
                                self.mr.read_u32(tag_address + 20) + 986
                            ),
                            zoom_min=self.mr.read_float(
                                self.mr.read_u32(tag_address + 20) + 988
                            ),
                            zoom_max=self.mr.read_float(
                                self.mr.read_u32(tag_address + 20) + 992
                            ),
                            autoaim_angle=self.mr.read_float(
                                self.mr.read_u32(tag_address + 20) + 996
                            ),  # radians, from unit_get_aim_assist_parameters()
                            autoaim_range=self.mr.read_float(
                                self.mr.read_u32(tag_address + 20) + 1000
                            ),
                            magnetism_angle=self.mr.read_float(
                                self.mr.read_u32(tag_address + 20) + 1004
                            ),
                            magnetism_range=self.mr.read_float(
                                self.mr.read_u32(tag_address + 20) + 1008
                            ),
                            deviation_angle=self.mr.read_float(
                                self.mr.read_u32(tag_address + 20) + 1012
                            ),
                            ...
                            tag_name=self.mr.read_string(
                                self.mr.read_u32(tag_address + 0x10)
                            ),
                    ...
                    biped_tag_address = self.mr.read_u32(
                        32 * (self.mr.read_u32(dynamic_player_address) & 0xFFFF)
                        + self.global_tag_instances_address
                        + 0x14
                    )
                    biped_camera_height_standing = self.mr.read_float(
                        biped_tag_address + 0x400
                    )
                    biped_camera_height_crouching = self.mr.read_float(
                        biped_tag_address + 0x404
                    )
                    crouchscale = self.mr.read_float(dynamic_player_address + 0x464)

                    player_object_debug["biped_tag_address"] = (
                        f"{hex(biped_tag_address)} -> {hex(self.mr.get_host_address(biped_tag_address))}"
                    )

                    # TODO: change to dataclasses instead of dicts?
                    player_object_data = dict(
                        flags=self.mr.read_u32(
                        ...
                        x=self.mr.read_float(dynamic_player_address + 0xC),
                        y=self.mr.read_float(dynamic_player_address + 0x10),
                        z=self.mr.read_float(dynamic_player_address + 0x14),
                        x_vel=self.mr.read_float(
                            dynamic_player_address + 0x18
                        ),  # object.translational_velocity
                        y_vel=self.mr.read_float(dynamic_player_address + 0x1C),
                        z_vel=self.mr.read_float(dynamic_player_address + 0x20),
                        legs_pitch=self.mr.read_float(
                        ...
                        legs_yaw=self.mr.read_float(
                            dynamic_player_address + 0x28
                        ),  # legs?
                        legs_roll=self.mr.read_float(
                            dynamic_player_address + 0x2C
                        ),  # legs?
                        pitch1=self.mr.read_float(
                            dynamic_player_address + 0x30
                        ),  # these get set in biped_snap_facing(), not sure what it is. (0, 0, 1) in most cases
                        yaw1=self.mr.read_float(dynamic_player_address + 0x34),
                        roll1=self.mr.read_float(dynamic_player_address + 0x38),
                        ang_vel_x=self.mr.read_float(dynamic_player_address + 0x3C),
                        ang_vel_y=self.mr.read_float(dynamic_player_address + 0x40),
                        ang_vel_z=self.mr.read_float(dynamic_player_address + 0x44),
                        aim_assist_sphere_x=self.mr.read_float(
                            dynamic_player_address + 0x50
                        ),  # center point? used in find_aim_assist_targets_recursive()
                        aim_assist_sphere_y=self.mr.read_float(
                            dynamic_player_address + 0x54
                        ),
                        aim_assist_sphere_z=self.mr.read_float(
                            dynamic_player_address + 0x58
                        ),
                        aim_assist_sphere_radius=self.mr.read_float(
                            dynamic_player_address + 0x5C
                        ),  # sphere radius? find_aim_assist_targets_recursive()
                        scale=self.mr.read_float(
                            dynamic_player_address + 0x60
                        ),  # object.scale (items only?)
                        type=self.mr.read_u16(dynamic_player_address + 0x64),
                        render_flags=self.mr.read_u16(dynamic_player_address + 0x66),
                        weapon_owner_team=self.mr.read_s16(
                            dynamic_player_address + 0x68
                        ),  # weapon.owner_team_index (e.g. ctf) -- also used in find_aim_assist_targets_recursive() for team check
                        powerup_unk2=self.mr.read_s16(dynamic_player_address + 0x6A),
                        idle_ticks=self.mr.read_s16(dynamic_player_address + 0x6C),
                        ...
                        max_health=self.mr.read_float(dynamic_player_address + 0x88),
                        max_shields=self.mr.read_float(dynamic_player_address + 0x8C),
                        health=self.mr.read_float(dynamic_player_address + 0x90),
                        shields=self.mr.read_float(dynamic_player_address + 0x94),
                        unk_dmg_countdown_0x98=self.mr.read_float(
                            dynamic_player_address + 0x98
                        ),  # starts counting down immediately
                        unk_dmg_countdown_0x9C=self.mr.read_float(
                            dynamic_player_address + 0x9C
                        ),
                        unk_dmg_countdown_0xA4=self.mr.read_float(
                        ...
                        unk_dmg_countdown_0xA8=self.mr.read_float(
                            dynamic_player_address + 0xA8
                        ),
                        unk3=self.mr.read_s32(
                        ...
                        shields_status_hex=hex(
                            self.mr.read_u16(dynamic_player_address + 0xB6)
                        ),
                        next_object=self.mr.read_s32(dynamic_player_address + 0xC4),
                        next_object_2=hex(
                            self.mr.read_u32(dynamic_player_address + 0xC8)
                        ...
                        parent_object=hex(
                            self.mr.read_s32(dynamic_player_address + 0xCC)
                        ),  # e.g. vehicle
                        ...
                        camo=self.mr.read_u8(
                            dynamic_player_address + 0x1B4
                        ),  # 65=nocamo (01000001), 81=camo (01010001)
                        flashlight=self.mr.read_u8(dynamic_player_address + 0x1B6),
                        current_action=self.mr.read_u32(
                        ...
                        xunk0=self.mr.read_float(
                            dynamic_player_address + 0x1D4
                        ),  # unknown, from biped_update_turning(), gets multiplied by leg rotation 24, 28, 2c.
                        yunk0=self.mr.read_float(dynamic_player_address + 0x1D8),
                        zunk0=self.mr.read_float(
                            dynamic_player_address + 0x1DC
                        ),  # z seems to stay at 0.0, but periodically will briefly flip to same z as others
                        xaima=self.mr.read_float(
                            dynamic_player_address + 0x1E0
                        ),  # unit vectors, -1 to 1 on x y z axes.
                        yaima=self.mr.read_float(dynamic_player_address + 0x1E4),
                        zaima=self.mr.read_float(dynamic_player_address + 0x1E8),
                        aiming_vector_x=self.mr.read_float(
                            dynamic_player_address + 0x1EC
                        ),  # used in first_person_camera_deterministic(), which gets used in player_aim_projectile()
                        aiming_vector_y=self.mr.read_float(
                            dynamic_player_address + 0x1F0
                        ),
                        aiming_vector_z=self.mr.read_float(
                            dynamic_player_address + 0x1F4
                        ),
                        xaim0=self.mr.read_float(
                            dynamic_player_address + 0x1F8
                        ),  # these seem to be used for projectiles -- see projectile_update()
                        yaim0=self.mr.read_float(dynamic_player_address + 0x1FC),
                        zaim0=self.mr.read_float(dynamic_player_address + 0x200),
                        xaim1=self.mr.read_float(
                            dynamic_player_address + 0x204
                        ),  # look in players_update_before_game() and unit_control()
                        yaim1=self.mr.read_float(dynamic_player_address + 0x208),
                        zaim1=self.mr.read_float(dynamic_player_address + 0x20C),
                        looking_vector_x=self.mr.read_float(
                            dynamic_player_address + 0x210
                        ),
                        looking_vector_y=self.mr.read_float(
                            dynamic_player_address + 0x214
                        ),
                        looking_vector_z=self.mr.read_float(
                            dynamic_player_address + 0x218
                        ),
                        move_forward=self.mr.read_float(
                            dynamic_player_address + 0x228
                        ),  # throttle?
                        move_left=self.mr.read_float(dynamic_player_address + 0x22C),
                        move_up=self.mr.read_float(
                        ...
                        melee_damage_type=self.mr.read_u8(
                        ...
                        animation_2=self.mr.read_u8(dynamic_player_address + 0x254),
                        animation_debug=self.get_animation_debug_info(
                            self.mr.read_u32(dynamic_player_address + 0x7C),
                            self.mr.read_s16(dynamic_player_address + 0x80),
                            self.mr.read_s16(dynamic_player_address + 0x82),
                        ),
                        selected_weapon_index=self.mr.read_s16(
                        ...
                        current_equipment=hex(
                            self.mr.read_u32(dynamic_player_address + 0x2C8)
                        ),
                        primary_nades=self.mr.read_u8(dynamic_player_address + 0x2CE),
                        secondary_nades=self.mr.read_u8(dynamic_player_address + 0x2CF),
                        zoom_level=self.mr.read_s8(dynamic_player_address + 0x2D0),
                        camo_amount=self.mr.read_float(
                        ...
                        camo_self_revealed=self.mr.read_u16(
                        ...
                        damagers_list_address=hex(
                            self.mr.get_host_address(dynamic_player_address + 0x3E0)
                        ),
                        ...
                        camera_x=self.mr.read_float(dynamic_player_address + 0xC),
                        camera_y=self.mr.read_float(dynamic_player_address + 0x10),
                        camera_z=(1 - crouchscale) * biped_camera_height_standing
                        + crouchscale * biped_camera_height_crouching
                        + self.mr.read_float(dynamic_player_address + 0x14),
                        air_1_0x64=self.mr.read_s16(
                            dynamic_player_address + 0x64
                        ),  # any_player_is_in_the_air() and unit_get_camera_position()
                        airborne=self.mr.read_u8(
                            dynamic_player_address + 0x424
                        ),  # &1 = airborne, &2 = slipping, 0 = standing, from biped_update()
                        landing_stun_current_duration=self.mr.read_u8(
                        ...
                        slipping_ticks=self.mr.read_u8(dynamic_player_address + 0x45A),
                        stop_ticks=self.mr.read_u8(dynamic_player_address + 0x45B),
                        jump_recovery_timer=self.mr.read_u8(
                            dynamic_player_address + 0x45C
                        ),
                        melee_animation_remaining=self.mr.read_u8(
                            dynamic_player_address + 0x45D
                        ),
                        melee_animation_damage_tick=self.mr.read_u8(
                            dynamic_player_address + 0x45E
                        ),  # from biped_update() and unit_cause_player_melee_damage()
                        melee_impact_this_tick=self.mr.read_u8(
                            dynamic_player_address + 0x45D
                        )
                        == self.mr.read_u8(
                            dynamic_player_address + 0x45E
                        ),  # TODO: move to computed?
                        landing=self.mr.read_u16(dynamic_player_address + 0x45F),
                        air_3_0x460=self.mr.read_s16(
                        ...
                        biped_flags=self.mr.read_u32(biped_tag_address + 0x2F4),
                        autoaim_pill_radius=self.mr.read_float(
                ...
                local_player = self.mr.read_s16(static_player_address + 0x2)
                ...
                player_stats = dict(
                    ...
                    name=(
                        "Steve"
                        # self.mr.read_bytes(static_player_address + 0x4, 24)
                        # .decode("utf-16")
                        # .split("\x00", 1)[0]
                        # if self.mr.use_pymem
                        # else b"".join(
                        #     [
                        #         int.to_bytes(i, signed=True)
                        #         for i in self.mr.read_bytes(
                    ...
                    action_seat=self.mr.read_u16(static_player_address + 0x2A),
                    respawn_timer=self.mr.read_u32(static_player_address + 0x2C),
                    respawn_penalty=self.mr.read_u32(static_player_address + 0x30),
                    object_ref=hex(
                        self.mr.read_u32(static_player_address + 0x34)
                    ),  # -1 when player is dead
                    object_index=self.mr.read_u16(static_player_address + 0x34),
                    object_id=self.mr.read_u16(static_player_address + 0x36),
                    previous_object_ref=hex(
                        self.mr.read_u32(static_player_address + 0x38)
                    ),  #  0x34 gets copied here when player dies
                    last_target_object_ref=hex(
                        self.mr.read_u32(static_player_address + 0x40)
                    ),  # set to same as copy above if no target
                    time_of_last_shot=self.mr.read_u32(static_player_address + 0x44),
                    player_speed=self.mr.read_float(static_player_address + 0x6C),
                    camo_timer=self.mr.read_u32(static_player_address + 0x68),
                    time_of_last_death=self.mr.read_u32(
                        static_player_address + 0x84
                    ),  # 0 at start of game
                    target_player_index=self.mr.read_u32(static_player_address + 0x88),
                    kill_streak=self.mr.read_u16(
                        static_player_address + 0x92
                    ),  # resets to 0 on death
                    multikill=self.mr.read_u16(
                        static_player_address + 0x94
                    ),  # resets to 0 on death
                    time_of_last_kill=self.mr.read_s16(
                        static_player_address + 0x96
                    ),  # in ticks, resets to -1 on death
                    kills=self.mr.read_s16(static_player_address + 0x98),
                    assists=self.mr.read_s16(static_player_address + 0xA0),
                    team_kills=self.mr.read_s16(static_player_address + 0xA8),
                    deaths=self.mr.read_s16(static_player_address + 0xAA),
                    suicides=self.mr.read_s16(static_player_address + 0xAC),
                    shots_fired=self.mr.read_s32(static_player_address + 0xAE),
                    shots_hit=self.mr.read_s16(static_player_address + 0xB2),
                    score=self.player_score_by_player_id(
                        player_index,
                        (
                            self.mr.read_u32(game_engine_globals_address + 0x4)
                            if game_engine_globals_address
                            else 0
                        ),
                    ),
                    ctf_score=self.mr.read_s16(static_player_address + 0xC4),
                    player_quit=self.mr.read_u8(
        ...
        game_info = dict(
            process_id=f"{self.mr.process_id} - {hex(self.mr.process_id)}",
            # pgcr_debug=dict(
            ...
            variant=self.mr.read_u8(0x2F90F4),
            global_stage=self.mr.read_string(
                0x2FAC20, length=63
            ),  # only populated for hostbox
            multiplayer_map_name=self.mr.read_string(
                0x2E37CD
            ...
            local_player_count=self.mr.read_u16(self.players_globals_address + 0x24),
            ...
            # fog_data=get_fog(),
            observer_cameras_address=f"{self.mr.get_host_address(0x271550):#x}",
            game_globals_address=f"{hex(self.game_globals_address)} -> {hex(self.mr.get_host_address(self.game_globals_address))}",
            game_globals_map_loaded=game_globals_map_loaded,
            players_are_double_speed=self.mr.read_u8(self.game_globals_address + 0x2),
            game_loading_in_progress=self.mr.read_u8(self.game_globals_address + 0x3),
            precache_map_status=self.mr.read_float(self.game_globals_address + 0x4),
            game_difficulty_level=self.mr.read_u8(self.game_globals_address + 0xE),
            ...
            global_random_seed=hex(self.mr.read_u32(0x2E3648)),
            stored_global_random=hex(
                self.mr.read_u32(self.game_globals_address + 16)
    ...
    def process_write_queue(self):
        ...
        while self.write_queue_from_ui.qsize() > 0:
            ...
            # print("before:", self.mr.get_formatted_bytes(0x9C514, 2))
            # self.mr.write_bytes(address, value, length)
            # print("after: ", self.mr.get_formatted_bytes(0x9C514, 2))
